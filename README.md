...
The check reports under four issue ids — name any of them in `warning +=` / `disable +=`:
`LuciqUnmaskedPii` (unmasked field) and `LuciqMaskingConfig` (project posture: auto-mask,
network, `FLAG_SECURE`, consent, old SDK) are **errors** that block; `LuciqUnmaskedPiiAdvisory`
and `LuciqMaskingConfigAdvisory` are non-blocking **warnings**.

Compliance level can also be set with the `LUCIQ_COMPLIANCE` env var. To switch warn/enforce
per run, gate `abortOnError` on a property:
